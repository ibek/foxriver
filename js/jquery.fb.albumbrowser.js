...
(function($) {
  $.fn.FacebookAlbumBrowser = function(options) {
    var defaults = {
      account: "",
      accessToken2: "EAAMRONZBWoUwBAO0dngS8jUGWmBq8WArTTGBmMOZA82ypqruhan9vcRvzEmMI5j5hGeOqLjT4PhYsMSoGCOJxamOKScqcCjnhiPjCT4Jws9ZCFimNiUxzHggb4OZBZB8yNkBijqZC9KhZC0iZAgCN2xoqLbmlvEnlLS0PtNM0sdoI5fpCJmG7SCzaWpY8zqmfYRJoZCMmZAZBkcEdUEvvA6Ema9pTfPySazdRgZD",
    ...
    var albumFields =
      "&fields=id,count,cover_photo,created_time,from,link,name,type,updated_time";
    var photoFields =
    ...
    selector.each(function(index) {
      ...
      if (settings.onlyAlbum == null) {
        $(container).append($('<ul>', {
          class: "fb-albums",
          "style": "min-height:" + settings.thumbnailSize +
            "px !important"
        }));
        ...
        $.ajax({
          ...
          success: function(result) {
            if ($(result.data).length > 0) {
              ...
              if (settings.albumsPageSize != null && settings.albumsPageSize >
                0) {
                var moreButton = $(container).find(".fb-btn-more");
                if (moreButton.length == 0) {
                  moreButton = $("<div>", {
                    class: "fb-btn-more fb-albums-more",
                    lang: "cz",
                    text: settings.albumsMoreButtonText
                  });
                  $(container).append(moreButton);
                  $(moreButton).click(function() {
                    var _this = $(this);
                    $(container).find("li.fb-album:hidden").slice(
                      0, settings.albumsPageSize).fadeIn(
                      function() {
                        if ($(container).find(
                            "li.fb-album:hidden").length ==
                          0) {
              ...
              for (a = 0; a < $(result.data).length; a++) {
                ...
                if (settings.skipAlbums.length > 0 && (settings.skipAlbums
                    .indexOf($(result.data).get(a).name) > -1 ||
                    settings.skipAlbums.indexOf($(result.data).get(
                      a).id.toString()) > -1)) {
                  continue;
                }
                if (settings.includeAlbums.length > 0 && (
                    settings.includeAlbums.indexOf($(result.data)
                      .get(a).name) < 0 && settings.includeAlbums
                    .indexOf($(result.data).get(a).id.toString()) <
                    0)) {
                  continue;
                }
                ...
                var albumListItem = $("<li>", {
                  ...
                  "style": "height:" + settings.thumbnailSize +
                    "px !important; width:" + settings.thumbnailSize +
                    "px !important"
                });
                if ($(result.data).get(a).count == null &&
                  settings.skipEmptyAlbums) {
                  continue;
                } else {
                  ...
                  $.ajax({
                    ...
                    cache: false,
                    data: {
                      album: $(result.data).get(a).id
                    },
                    dataType: 'jsonp',
                    success: function(cover) {
                      var listItem = $(
                        "li.fb-album[data-id='" +
                        getParameterByName("album", $(
                          this).get(0).url) + "'] ");
                      if (!$(cover).get(0).error && $(cover)
                        .get(0).images) {
                        ...
                        for (i = 0; i < $(cover.images).length; i++) {
                          if (
                            ($(cover.images)[i].height >=
                              prefHeight && $(cover.images)[
                                i].width >= prefWidth) &&
                            ($(cover.images)[i].height <=
                              albumImg.height && $(cover.images)[
                                i].width <= albumImg.width)
                          ) {
                        ...
                        var albumThumb = $("<img>", {
                          style: "margin-left:" + (
                            prefWidth - albumImg.width
                          ) / 2 + "px; display:none;",
                          "data-id": $(cover).get(0).id,
                          class: "fb-album-thumb",
                          "data-src": albumImg.source,
                          src: ""
                        })
                  ...
                  albumListItem.append($("<div>", {
                    class: "fb-album-title",
                    text: $(result.data).get(a).name
                  }));
                  if (settings.showImageCount) {
                    albumListItem.append($("<div>", {
                      class: "fb-album-count",
                      text: $(result.data).get(a).count
                    }));
                  }
                  $(albumList).append(albumListItem);
                  $(albumList).removeClass("fb-loading-image");
                  if (settings.albumsPageSize > 0 && $(albumList)
                    .find("li").index(albumListItem) >= settings.albumsPageSize
                  ) {
                    albumListItem.hide();
                  ...
                  $(albumListItem).click(function() {
                    var self = $(this);
                    $(selector).append($("<div>", {
                      class: "fb-album-preview"
                    }));
                    $(selector).find("div.fb-albums-more").hide();
                    var previewContainer = selector.find(
                      ".fb-album-preview");
                    ...
                    $(previewContainer).append($("<ul>", {
                      class: "fb-photos",
                      "style": "min-height:" + settings
                        .thumbnailSize +
                        "px !important"
                    }));
                    photosContainer = $(previewContainer).find(
                      "ul.fb-photos");
                    ...
                    $(selector).find("ul.fb-albums").fadeOut(
                      function() {
                  ...
                  $(albumListItem).hover(
                    ...
                    function() {
                      var self = $(this),
                        height = self.find("div.fb-album-title")
                        .height(),
                        padding = 10;
              ...
              if (result.paging && result.paging.next && result.paging
                .next != "") {
      ...
      function loadPhotos(url, container) {
        ...
        if (previewContainer.find(".fb-photo").length == 0) {
          ...
          if (settings.showAlbumNameInPreview) {
            $.ajax({
              type: 'GET',
              url: url.replace("/photos", "/").replace(photoFields,
                "&fields=name"),
              cache: false,
              dataType: 'jsonp',
              success: function(albumData) {
                if (albumData != null) {
                  previewContainer.find(".fb-album-heading").text(
                    albumData.name);
                }
              }
            });
        ...
        $.ajax({
          ...
          success: function(result) {
            ...
            if (result.data.length > 0) {
              ...
              if (settings.photosPageSize != null && settings.photosPageSize >
                0) {
                var moreButton = $(container).parent().find(
                  ".fb-btn-more");
                if (moreButton.length == 0 && settings.showMoreButton) {
                  moreButton = $("<div>", {
                    class: "fb-btn-more fb-photos-more",
                    lang: "cz",
                    text: settings.photosMoreButtonText
                  });
                  $(container).parent().append(moreButton);
                  $(moreButton).click(function() {
                    var _this = $(this);
                    $(container).find("li.fb-photo:hidden").slice(
                      0, settings.photosPageSize).fadeIn(
                      function() {
                        if ($(container).find(
                            "li.fb-photo:hidden").length ==
                          0) {
                          _this.fadeOut(function() {
                            _this.remove();
                          });
              ...
              for (a = 0; a < result.data.length; a++) {
                ...
                for (i = 0; i < $(result.data)[a].images.length; i++) {
                  if (
                    ($(result.data)[a].images[i].height >=
                      prefHeight && $(result.data)[a].images[i].width >=
                      prefWidth) &&
                    ($(result.data)[a].images[i].height <=
                      albumImg.height && $(result.data)[a].images[
                        i].width <= albumImg.width)
                  ) {
                ...
                var photoLink = $("<a>", {
                  class: "fb-photo-thumb-link",
                  href: $(result.data)[a].source,
                  "data-fb-page": $(result.data)[a].link
                });
                var marginWidth = 0;
                ...
                if (settings.photosCheckbox) {
                  ...
                  imageCheck.click(function() {
                    var eventObj = {
                      id: $(this).parent().find(
                        "img.fb-photo-thumb").attr(
                        "data-id"),
                      url: $(this).parent().find("a").attr(
                        "href"),
                      thumb: $(this).parent().find(
                        "img.fb-photo-thumb").attr("src")
                    };
                    $(this).toggleClass("checked");
                    if ($(this).hasClass("checked")) {
                      if (settings.photoChecked != null &&
                        typeof(settings.photoChecked) ==
                        "function") {
                        settings.checkedPhotos.push(eventObj);
                        settings.photoChecked(eventObj);
                      }
                    } else {
                      if (settings.photoUnchecked != null &&
                        typeof(settings.photoUnchecked) ==
                        "function") {
                        for (i = 0; i < settings.checkedPhotos
                          .length; i++) {
                          if (settings.checkedPhotos[i].id ==
                            eventObj.id) {
                            settings.checkedPhotos.splice(i,
                              1);
                ...
                var photoThumb = $("<img>", {
                  style: "margin: auto;position: absolute;top: 0; left: 0; bottom: 0; right: 0;display:none;",
                  "data-id": $(result.data).get(a).id,
                  class: "fb-photo-thumb",
                  "data-src": albumImg.source,
                  src: ""
                });
                var photoText = $("<span>", {
                ...
                if (settings.photosCheckbox) {
                  ...
                  for (i = 0; i < settings.checkedPhotos.length; i++) {
                    if (settings.checkedPhotos[i].id == $(result.data)
                      .get(a).id) {
                ...
                if (settings.photosPageSize > 0 && $(container).find(
                    "li").index(photoListItem) >= settings.photosPageSize) {
                  photoListItem.hide();
                ...
                initLightboxes(container.find(
                  "a.fb-photo-thumb-link"));
              }
          ...
          success: function(result) {
            ...
            if (result.data.length > 0) {
              if (result.paging.next != "") {
                var commentsMore = $(container).find(
                  ".fb-comment-more");
                if (commentsMore.length == 0) {
                  commentsMore = $("<div/>", {
                    "class": "fb-comment-more",
                    text: "more"
                  });
                ...
                $(commentsMore).click(function() {
                  loadComments(objectId, container, $(this).attr(
                    "data-next"));
                  return false;
                });
              } else {
              ...
              $(commentsMore).css("maxWidth", container.find(
                ".fb-preview-img").width() - 12);

              for (c = 0; c < result.data.length; c++) {
                var accountIcon = openGraphApiUrl + $(result.data)[
                  c].from.id + "/picture?type=square";
                var comment = $("<div/>", {
                  "class": "fb-comment"
                });
                comment.css("maxWidth", container.find(
                  ".fb-preview-img").width() - 12);
                comment.append($("<img/>", {
                  src: accountIcon,
                  "class": "fb-comment-account"
                }));
                var commentText = $("<div/>", {
                ...
                commentText.append($("<a/>", {
                  "class": "fb-comment-account",
                  target: "_blank",
                  href: "http://facebook.com/" + $(result.data)[
                    c].from.id,
                  text: $(result.data)[c].from.name
                }));
                commentText.append($("<div/>", {
                  "class": "fb-comment-date",
                  text: parseDateString($(result.data)[c].created_time)
                    .toLocaleString()
                }));
                commentText.append($("<div/>", {
                  text: $(result.data)[c].message
                }));
              ...
              if (nextUrl == null) {
                ...
                $(".fb-preview-overlay").animate({
                  scrollTop: $(container).find(".fb-comment")
                    .last().offset().top
                }, 1000);
              }
            }
      ...
      function loadIfVisible(photoThumb) {
        ...
        if (element != null) {
          if (isScrolledIntoView($(element)) && $(photoThumb).attr(
              "src") == "") {
      ...
      function addAccountInfo(container) {
        var accountInfoContainer = $("<div>", {
          class: "fb-account-info"
        });
        var accountInfoLink = $("<a>", {
          target: "_blank",
          href: ""
        });
        accountInfoContainer.append(accountInfoLink);
        accountInfoLink.append($("<img>", {
          src: openGraphApiUrl + settings.account +
            "/picture?type=square"
        }));
        accountInfoLink.append($("<h3>", {
          class: "fb-account-heading"
        }));
        ...
        $.ajax({
          ...
          dataType: 'jsonp',
          success: function(result) {
            $(container).find(".fb-account-info").find(
              ".fb-account-heading").text(result.name);
            $(container).find(".fb-account-info").find("a").attr(
              "href", "http://facebook.com/" + (!result.username ?
      ...
      function addLikeButton(container, url) {
        if (settings.likeButton) {
          var likeBtn = $("<div>", {
            ...
            "data-share": settings.shareButton,
            "data-show-faces": "false"
          });
          var likeBtnContainer = $("<div>", {
            class: "fb-like-container"
          });
        ...
        if (settings.shareButton && settings.addThis != null &&
          settings.addThis != "") {
          //Add this
          var toolboxElement = $("<div>", {
            "data-url": url,
            class: "addthis_toolbox addthis_default_style"
          });
          for (btn = 1; btn <= 4; btn++) {
            toolboxElement.append($("<a>", {
              class: "addthis_button_preferred_" + btn
            }));
          }
          toolboxElement.append($("<a>", {
      ...
      function initLightboxes(photoLink) {
        ...
        if (overlay.length == 0) {
          overlay = $("<div>", {
            class: "fb-preview-overlay"
          });
          var lightboxContent = $("<div>", {
            class: "fb-preview-content"
          });

          overlay.append(lightboxContent);
          lightboxContent.append($("<img>", {
            class: "fb-preview-img"
          }));
          if (settings.showImageText || settings.likeButton || settings
            .shareButton) {
            lightboxContent.append($("<div>", {
              class: "fb-preview-text"
            }));
          }
          overlay.append($("<img>", {
            class: "fb-preview-img-prev",
            src: settings.pluginImagesPath + "prev-icon.png"
          }));
          overlay.append($("<img>", {
            class: "fb-preview-img-next",
            src: settings.pluginImagesPath + "next-icon.png"
          }));
        ...
        $(photoLink).click(function(event) {
          ...
          if (settings.showImageText) {
            previewText.html(parseLinks($(this).find(
              ".fb-photo-text").text()));
          }
          ...
          var eventObj = {
            ...
            url: $(this).attr("href"),
            thumb: $(this).find("img.fb-photo-thumb").attr(
              "data-id")
          };
          if (settings.photoSelected != null && typeof(settings.photoSelected) ==
            "function") {
          ...
          if (settings.lightbox) {
            ...
            previewImage.hide();
            overlay.find(
              "img.fb-preview-img-prev,img.fb-preview-img-next").hide();
            previewImage.attr("data-id", $(this).find(
              "img.fb-photo-thumb").attr("data-id"));
            ...
            previewImage.load(function() {
              ...
              $(".fb-comment,.fb-comment-more").css("maxWidth",
                $(this).width() - 12);
              $(this).show();


              if (settings.showComments) {
                loadComments($(this).attr("data-id"), $(this).parent(),
                  null);
              }

              var prevImg = overlay.find(
                "img.fb-preview-img-prev");
              prevImg.show();
              prevImg.unbind("click");
              prevImg.click(function() {
                var currentImageContainer = $(this).parent()
                  .find(".fb-preview-content");
                var currentImage = $(currentImageContainer)
                  .find(".fb-preview-img");
                var currentImageLinkItem = $("[href='" +
                ...
                if (currentImageLinkItem.length != 0) {
                  ...
                  if (prev.length != 0) {
                    prevImg = prev.find(
                      ".fb-photo-thumb-link");
                    previewImage.attr("data-id", prevImg.find(
                      "img.fb-photo-thumb").attr(
                      "data-id"));
                    previewImage.attr("src", prevImg.attr(
                      "href"));
                  } else {
                    prevImg = currentImageLinkItem.parent()
                      .parent().find("li").last().find(
                        ".fb-photo-thumb-link");
                    previewImage.attr("data-id", prevImg.find(
                      "img.fb-photo-thumb").attr(
                      "data-id"));
                  ...
                  addLikeButton(previewText, prevImg.attr(
                    "data-fb-page"));
                }
                return false;
              });

              var nextImg = overlay.find(
                "img.fb-preview-img-next");
              nextImg.show();
              ...
              nextImg.click(function() {
                ...
                if (currentImageLinkItem.length != 0) {
                  ...
                  if (next.length != 0) {
                    nextImg = next.find(
                      ".fb-photo-thumb-link");
                    previewImage.attr("data-id", nextImg.find(
                      "img.fb-photo-thumb").attr(
                      "data-id"));
                    previewImage.attr("src", nextImg.attr(
                      "href"));
                  } else {
                    nextImg = currentImageLinkItem.parent()
                      .parent().find("li").first().find(
                        ".fb-photo-thumb-link");
                    previewImage.attr("data-id", nextImg.find(
                      "img.fb-photo-thumb").attr(
                      "data-id"));
                  ...
                  addLikeButton(previewText, nextImg.attr(
                    "data-fb-page"));
                }
                return false;
              });
